...
export default function Pagination(props) {
  ...
  for (
    let i = 1;
    i <= Math.ceil(props.totalOrders / props.ordersPerPage);
    i++
  ) {
    pageNumbers.push(i);
  }
  ...
  useEffect(() => {
    if (pageNumbers.length <= 3) {
      ...
      setDigitsToShow([
        ...pageNumbers.slice(props.currentPage - 2, props.currentPage),
        ...pageNumbers.slice(props.currentPage, props.currentPage + 1),
      ]);
    }
  }, [props.currentPage]);
        ...
        {digitsToShow.map((number) => (
          <li className="page-item" key={number}>
            <button
              type="button"
              className={
                (props.currentPage === number && "bg-red-600 text-white ") +
                "page-link px-3 py-1.5 rounded-md text-xl"
              }
