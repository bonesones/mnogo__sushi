import {useState} from "react";
import axios from "axios";
import {Link} from "react-router-dom";

export default function Promotion({ promotion, promotions, setPromotions, setOpenModal }) {

    const [openDeleteSubmit, setOpenDeleteSubmit] = useState(false);

    const handleDelete = function () {
        setOpenDeleteSubmit(true)
    }

    const handleCancel = function () {
        setOpenDeleteSubmit(false)
    }

    const handleSubmitDelete = async function (e) {
        e.preventDefault()
        try {
            await axios.delete(`/api/promotion/delete/${promotion.id}`, {
                withCredentials: true
            })

            setPromotions(prev => [...prev.filter(({id}) => id !== promotion.id)])
            setOpenModal(true)
            setTimeout(() => setOpenDeleteSubmit(false), 2000);
        } catch (e) {
            console.log(e)
        }
    }

    return (
        <article className="flex flex-col gap-5 w-80">
            {openDeleteSubmit && (
                <div className="fixed  z-20 flex flex-col border-2 border-red-600 items-center gap-3 top-1/2 left-1/2 rounded-md -translate-x-1/2 -translate-y-1/2 bg-white w-5/6 md:w-120 px-6 py-4">
                    <p className="text-center">Вы точно хотите удалить продукт?</p>
                    <form
                        className="flex flex-col gap-5 mt-4 mb-2"
                        onSubmit={(e) => handleSubmitDelete(e)}
                    >
                        <input
                            type="submit"
                            className="bg-second text-white px-12 py-2 rounded-md"
                            value="Да, удалить"
                        />
                        <button
                            type="button px-12 py-2"
                            onClick={() => handleCancel()}
                        >
                            Нет, я передумал
                        </button>
                    </form>
                </div>
            )}
            <img src={"http://192.168.1.156:3000/" + promotion.image} width="100%" />
            <h2 className="text-xl font-semibold text-center">{promotion.title}</h2>
            <p className="text-center">{promotion.description}</p>
            <div className="flex gap-8 justify-center">
                <button type="button" className="bg-second px-4 py-1 rounded-md text-white" onClick={handleDelete}>Удалить</button>
                <Link to={'/admin/promotions/promotion_edit/' + promotion.id}>Редактировать</Link>
            </div>
        </article>
    );
}
