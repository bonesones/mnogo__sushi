...
export default function Promocode ({ promocode, setPromocodes, setOpenModal }) {
    ...
    const handleSubmitDelete = async function (e) {
        e.preventDefault()
        try {
            const response = await axios.delete(`/api/promocode/delete/${promocode.id}`, {
                withCredentials: true
            })
            setPromocodes((prev) => [...prev.filter(({id}) => id !== promocode.id)])
            setOpenModal(true)
            setTimeout(() => {
                setOpenModal(false)
            }, 2000)
    ...
    return (
        <div key={promocode.id} className="flex flex-col gap-2 items-center w-full">
            ...
            <div className="flex justify-between w-full">
                <p>Тип скидки</p>
                <p>{promocode.discount_type === "fix" ? "Фиксированная" : "Процентная"}</p>
            </div>
            <div className="flex justify-between w-full">
                <p>Скидка</p>
                <p>{promocode.discount_amount + " " + (promocode.discount_type === "fix" ? "₽" : "%")}</p>
            </div>
            <div className="flex gap-8 justify-center">
                <button type="button" className="bg-second px-4 py-1 rounded-md text-white"
                        onClick={handleDelete}>Удалить
                </button>
                <Link to={'/admin/promocodes/promocode_edit/' + promocode.id}>Редактировать</Link>
            </div>
