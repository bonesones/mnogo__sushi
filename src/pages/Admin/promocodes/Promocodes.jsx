            ...
            <div className="mt-12 w-full flex flex-col gap-12 max-w-[50rem]">
                {
                    promocodes.length > 0 ? (
                        promocodes.map((promocode) => (
                            <Promocode key={promocode.id} promocode={promocode} setPromocodes={setPromocodes} setOpenModal={setOpenModal} />
                        ))
                    ) : (
                        <h2 className="text-xl text-center mt-12 mb-12 font-semibold">Актуальные промокоды отсутствуют</h2>
                    )
                }
            </div>
        </div>
