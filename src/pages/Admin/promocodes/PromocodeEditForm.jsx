...
export default function PromocodeEditForm () {
    ...
    useEffect(() => {
        const fetchPromocode =  async () => {
            try {
                const response = await axios.get(`/api/promocode/getone/byid/${promocodeId}`)
                setCurrentPromocode(response.data)
                setError('')
                setCurrentDiscountType({
                    value: response.data.discount_type,
                    label: response.data.discount_type === "fix" ? "Фиксированный" : "Плавающий"
                })
            } catch (e) {
    ...
    const onSubmit = async function (data) {
        try {
            const response = await axios.put(`/api/promocode/update/${promocodeId}`, {
                ...data,
    ...
    return (
        <div className="mt-16 mb-16 flex flex-col items-center">
            {openModal && (
                <div
                    className="fixed top-10 left-1/2 sm:text-lg -translate-x-1/2 bg-red-400 px-6 sm:px-10 w-fit rounded-md text-white py-5 sm:py-8">
                    Промокод сохранен!
                </div>
            )}
            <div className="w-full flex gap-4 mb-12 font-medium">
                <Link to="/admin/promocodes">Промокоды</Link>><span>Редактировать промокод</span>
            </div>
            ...
            >
                ...
                <Select options={options} value={currentDiscountType} onChange={(e) => setCurrentDiscountType(e)}/>
                ...
                <div className="flex items-center gap-4">
                    <input
                        {...register("discount_amount", {
                            required: true,
                            validate: {
                                maxFixValue: value => Number(value) <= Number(getValues('min_amount')) || "error",
                                maxPercentValue: value => currentDiscountType.value === "fix" ? true : Number(value) <= 100 || "error"
                            }
                        })}
                        type="number"
                        min={1}
                ...
                {Object.keys(errors).length > 0 &&
                    <span className="text-red-600">Проверьте правильность заполнения полей!</span>}
                {error && <span className="text-red-600">{error}</span>}
                <input type="submit" value="Создать промокод"
