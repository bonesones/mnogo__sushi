...
export default function Callback({ callback, options }) {
  ...
  const handleSubmit = async function () {
    try {
      const response = await api.put(
        `/api/callback/update/${callback.id}`,
        {
  ...
  return (
    <div
      key={callback.id}
      className="flex flex-col border-2 gap-4 p-10 border-red-500 rounded-md"
    >
      <div className="flex flex-col md:flex-row md:w-full justify-between md:items-center gap-4">
        <div className="flex gap-2 items-center">
          <span className="bg-second px-2 py-1 rounded-md text-white">
            Статус
          </span>
          {currentStatus === "Новая" ? (
            <Select
        ...
        <span>
          Дата заявки: {new Date(callback.createdAt).toLocaleDateString("ru")}
        </span>
        <div className="flex flex-col gap-2">
          <span>Имя: {callback.name}</span>
          <span>
            Телефон:{" "}
            <a className="font-medium" href={"tel:" + callback.phone}>
              {callback.phone}
            </a>
          </span>
        </div>
