...
export default function ProductDetails({ active, data, handleChange }){
    ...
    return(
            <article className={(active ? "" : "hidden ") + "w-96 sm:w-120 lg:w-232 h-160 product-details fixed z-20 px-12 sm:px-20 md:px-12 py-7 lg:box-content flex flex-col overflow-y-auto rounded-md"}>
                ...
                <div className="flex flex-col lg:gap-8 items-center lg:mt-12">
                    <div className="flex flex-col  lg:flex-row items-center lg:gap-8">
                        <img className="mt-8 lg:mt-0 h-fit sm:w-10/12 md:w-8/12 lg:w-full lg:h-96" src={data.image} />
                        {data.contains && <div className="flex flex-col items-center w-full lg:h-96">
                           <div className="mx-auto mt-12 lg:mt-0 flex flex-col gap-8 overflow-y-auto h-full">
                                                {
                                                    data.contains.map(({ name, img, description }, key) => {
                                                        return (
                                                            <ProductDetailsContainsCard title={name}
                                                            image={img}
